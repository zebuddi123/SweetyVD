...
CompilerIf #PB_Compiler_IsMainFile
		...
		CompilerCase #PB_OS_Windows
			...
			Procedure StartPrefs()
				...
				If OpenPreferences("SweetyVD.ini", #PB_Preference_GroupSeparator) = 0   ;Create Default SweetyVD.ini setting
					...
					;---------------------------------------------------------  Zebuddi.
					PreferenceComment("Pre/Post fixes for Variables  --- Zebuddi.")
					PreferenceGroup("PrePostFix")
					WritePreferenceLong("PostFixVal",  1)
					WritePreferenceString("PreFix", "i")
					WritePreferenceString("PostFix", ".i")
					;---------------------------------------------------------
					
					;Pre and Post Fixes to Vaiables
	...
	Procedure.s sMakeLength(sString.s, iPos.i)
	...
	Procedure CodeCreate(Dest.s = "Clipboard")
		...
		Protected Mini.i, Maxi.i, TmpTabName.s, ActiveTab.i=-1, TmpConstants.s, FirstPass.b, I.i, J.i
		
		;---------------------------------------------------------  Zebuddi.
		Protected sPreF.s, sPost.s, sCheckPreFix.s, iVNameLength.i  ; Zebuddi
				
		If OpenPreferences("SweetyVD.ini", #PB_Preference_GroupSeparator) 
			PreferenceGroup("PrePostFix")
			sPreF = ReadPreferenceString("PreFix",  "")
			sPost = ReadPreferenceString("PostFix", "")
			sCheckPreFix = GetGadgetText(#StringGadget_PreFix)
			If Bool( sCheckPreFix > #Null$)
				If sCheckPreFix <> sPreF
					WritePreferenceString("PreFix", sCheckPreFix)
					sPreF = ReadPreferenceString("PreFix",  "")
				EndIf
			EndIf	
		EndIf	
		...
		For I = 0 To ArraySize(Buffer())
			Buffer(I)\VName = sPreF + Mid(Buffer(I)\Name, 2)
			If Len(Buffer(I)\Vname) > iVNameLength
				iVNameLength = Len(Buffer(I)\Vname)
			EndIf	
		Next
		
		For I = 0 To ArraySize(Buffer())
			Buffer(I)\Vname = sMakeLength(Buffer(I)\Vname, iVNameLength)
		Next	
		;---------------------------------------------------------
		...
		If GetGadgetState(#CodeEnumeration) = #True   ;-Include Enumeration
			With Buffer(0)
				...
				Else
					Code + "Global " + Trim(\Vname) + sPost +#CRLF$+#CRLF$
				EndIf
			EndWith
		;---------------------------------------------------------	
			If GetGadgetState(#CodeStatusBar) = #True
				If GetGadgetState(#CodeConstants) = #True
					...
					Code +#TAB$+ "#StatusBar_0" +#CRLF$
					Code + "EndEnumeration" +#CRLF$+#CRLF$
				Else
					;---------------------------------------------------------  Zebuddi.
					Code + "Global " + sMakeLength(sPreF + "StatusBar_0.i", iVNameLength) +#CRLF$+#CRLF$
					;---------------------------------------------------------
				EndIf
			EndIf
			...
			For I=1 To ArraySize(Buffer())
				With Buffer(I)
					...
					If GetGadgetState(#CodeConstants)
						Code +#TAB$+ \Name +#CRLF$
					Else
						Code + "Global " + Trim(\Vname) + sPost  +#CRLF$
					EndIf
				EndWith
			Next
			If GetGadgetState(#CodeConstants)
			...
			If ArraySize(ImageBtnPathArray()) > 0   ;Image Enumeration
				...
				For I = 1 To ArraySize(ImageBtnPathArray())
					If FileSize(ImageBtnPathArray(I)\ImagePath)
						If GetGadgetState(#CodeConstants)
							Code +#TAB$+ "#Img_Window_" + Str(I) +#CRLF$
						Else
							Code + "Global " + sPreF + "Img_Window_" + Str(I) + sPost +#CRLF$
						EndIf
					EndIf
				Next
				If GetGadgetState(#CodeConstants)
			...
			If ArraySize(ImageBtnPathArray()) > 0   ;Image Enumeration
				...
				For I = 1 To ArraySize(ImageBtnPathArray())   ;LoadImage
					If FileSize(ImageBtnPathArray(I)\ImagePath)
						...
						Else
							Code + "Global " + sPreF + "Img_Window_" + Str(I) + sPost + " = LoadImage(#PB_Any, " + #DQUOTE$ + ImageBtnPathArray(I)\ImagePath + #DQUOTE$ + ")" +#CRLF$
						EndIf
					EndIf
				Next
				Code +#CRLF$
		...
		;-Create Window
		With Buffer(0)
			If GetGadgetState(#CodeConstants) : Name = \Name : Else : Name = \Vname : EndIf
			...
			If GetGadgetState(#CodeConstants)
				Code +#TAB$+ "If OpenWindow("+Name+", X, Y, Width, Height, "
			Else
				Code +#TAB$+ Name + " = OpenWindow(#PB_Any, X, Y, Width, Height, "
			EndIf
			...
			If GetGadgetState(#CodeConstants) = #False : Code +#TAB$+ "If " + Name +#CRLF$ :EndIf
			
			If Left(\BackColor, 5) <> "#Nooo" And \BackColor <> ""   ;Background window color
				Code +#TAB$+#TAB$+ "SetWindowColor(" + Name + ", " + \BackColor + ")" +#CRLF$
			EndIf
		EndWith
		
		If GetGadgetState(#CodeStatusBar)   ;Add a Status Bar Enumeration
			If GetGadgetState(#CodeConstants)
				Code +#TAB$+#TAB$+ "CreateStatusBar(#StatusBar_0, WindowID(" + Name + "))" +#CRLF$
			Else
				Code +#TAB$+#TAB$+ sPreF + "StatusBar_0 = CreateStatusBar(#PB_Any, WindowID(" + Name + "))" +#CRLF$
			EndIf
		EndIf
		...
		For I=1 To ArraySize(Buffer())
			With Buffer(I)
				...
				If Model = "ScintillaGadget"   ;Specific ScintillaGadget: If InitScintilla() : ScintillaGadget(xxx) : EndIf
					Code +#TAB$+#TAB$+ "If InitScintilla()" +#CRLF$+#TAB$
				EndIf
				
				If GetGadgetState(#CodeConstants)
					Code +#TAB$+#TAB$+ Model + "(" + Name + ", " + X + ", " + Y + ", " + Width + ", " + Height   ;Common part
				Else
					Code +#TAB$+#TAB$+ Name  + " = " + Model + "(#PB_Any, " + X + ", " + Y + ", " + Width + ", " + Height   ;Common part
				EndIf
				...
				Select Model
					Case "ComboBoxGadget"
						Code +#TAB$+#TAB$+ "AddGadgetItem("+ Name + ", -1, " + #DQUOTE$ + Mid(\Name,2) + #DQUOTE$ + ")" +#CRLF$
						Code +#TAB$+#TAB$+ "SetGadgetState("+ Name + ", 0)" +#CRLF$
					Case "EditorGadget"
						Code +#TAB$+#TAB$+ "AddGadgetItem("+ Name + ", -1, " + #DQUOTE$ + Mid(\Name,2) + #DQUOTE$ + ")" +#CRLF$
					Case "IPAddressGadget"
						Code +#TAB$+#TAB$+ "SetGadgetState("+ Name + ", MakeIPAddress(127, 0, 0, 1))" +#CRLF$
					Case "PanelGadget"
						For J=0 To CountString(Caption, "|")   ;Draw Tab + Text (eg: "Tab1|Tab2(x)|Tab3"
							...
							If TmpTabName <> ""
								If Right(TmpTabName, 3) = "(x)"
									...
									Code +#TAB$+#TAB$+ "AddGadgetItem("+ Name + ", -1, " + #DQUOTE$ + TmpTabName + #DQUOTE$ + ")" +#CRLF$
									ActiveTab = J
								Else
									Code +#TAB$+#TAB$+ "AddGadgetItem("+ Name + ", -1, " + #DQUOTE$ + TmpTabName + #DQUOTE$ + ")" +#CRLF$
								EndIf
						...
						Code +#TAB$+#TAB$+ "SetGadgetState(" + Name + ", " + ActiveTab + ")" +#CRLF$
					Case "ScintillaGadget"
						: Code +#TAB$+#TAB$+ "EndIf" +#CRLF$
					Case "ProgressBarGadget", "SpinGadget", "TrackBarGadget"   ;Set progression 2/3 Cosmetic
						...
						Code +#TAB$+#TAB$+ "SetGadgetState("+ Name + ", " + Str(Mini+(Maxi-Mini)*2/3) + ")" +#CRLF$
					Case "TabBarGadget"
						Code +#TAB$+#TAB$+ "AddTabBarGadgetItem("+ Name + ", #PB_Default, " + #DQUOTE$ + Caption + #DQUOTE$ + ")" +#CRLF$
				EndSelect
				
				If Left(\FrontColor, 5) <> "#Nooo" And \FrontColor <> ""
					Code +#TAB$+#TAB$+ "SetGadgetColor(" + Name + ", #PB_Gadget_FrontColor, " + \FrontColor + ")" +#CRLF$
				EndIf
				If Left(\BackColor, 5) <> "#Nooo" And \BackColor <> ""
					Code +#TAB$+#TAB$+ "SetGadgetColor(" + Name + ", #PB_Gadget_BackColor, " + \BackColor + ")" +#CRLF$
				EndIf
				
				If \ToolTip <> "#Nooo" And \ToolTip <> ""
					Code +#TAB$+#TAB$+ "GadgetToolTip(" + Name + ", " + #DQUOTE$ + \ToolTip + #DQUOTE$ + ")" +#CRLF$
				EndIf
				
				Select Model
					Case "ContainerGadget", "PanelGadget", "ScrollAreaGadget"
						Code +#TAB$+#TAB$+ "CloseGadgetList()" +#CRLF$
				EndSelect
		...
		If GetGadgetState(#CodeEventLoop) = #True   ;-Include Event Loop
			Code + "Repeat" +#CRLF$
			;---------------------------------------------------------  Zebuddi.
			Code +#TAB$+ "iEvent = WaitWindowEvent()" +#CRLF$
			Code +#TAB$+ "Select iEvent" +#CRLF$
			;---------------------------------------------------------
			Code +#TAB$+#TAB$+ "Case #PB_Event_Menu" +#CRLF$
			Code +#TAB$+#TAB$+#TAB$+ "Select EventMenu()" +#CRLF$
			Code +#TAB$+#TAB$+#TAB$+ "EndSelect" +#CRLF$+#CRLF$
			Code +#TAB$+#TAB$+ "Case #PB_Event_Gadget" +#CRLF$
			Code +#TAB$+#TAB$+#TAB$+ "Select EventGadget()" +#CRLF$
			For I=1 To ArraySize(Buffer())
				If Buffer(I)\ModelType = 9 : Break : EndIf   ;Gadget Deleted
				If Buffer(I)\Type = 1						 ;Button
					If GetGadgetState(#CodeConstants)
						Code +#TAB$+#TAB$+#TAB$+#TAB$+ "Case " + Buffer(I)\Name +#CRLF$
					Else
						Code +#TAB$+#TAB$+#TAB$+#TAB$+ "Case " + Buffer(I)\VName +#CRLF$
					EndIf
				EndIf
			Next
			Code +#TAB$+#TAB$+#TAB$+ "EndSelect" +#CRLF$+#CRLF$
			Code +#TAB$+#TAB$+ "Case #PB_Event_CloseWindow" +#CRLF$
			Code +#TAB$+#TAB$+#TAB$+ "End" +#CRLF$
			Code +#TAB$+ "EndSelect" +#CRLF$
			Code + "ForEver" +#CRLF$
		EndIf
...
Procedure OpenMainWindow(Designer_Width, Designer_Height, X = 0, Y = 0)
	...
	If OpenPreferences("SweetyVD.ini", #PB_Preference_GroupSeparator)
		...
		;---------------------------------------------------------  Zebuddi.
		PreferenceGroup("PrePostFix")
		Global gsPrefix.s  = ReadPreferenceString("PreFix", "") ; Get the defined prefix
		Global giPostFix.i = ReadPreferenceLong("PostFixVal", 0); get the defined postfix value 1 for postfixing : 0 no postfixing
																;---------------------------------------------------------
		ClosePreferences()
	EndIf
	...
	TextGadget(#TextGadget_PreFix, 525, 4, 60, 20, "Var/PreFix")
	StringGadget(#StringGadget_PreFix, 590, 1, 50, 20,"")
	CheckBoxGadget(#CheckBox_PostFix, 645, 1, 70, 20, "PostFix") 
	CloseGadgetList()
	CallDebugger
	;---------------------------------------------------------  Zebuddi.
	SetGadgetText(#StringGadget_PreFix, gsPrefix)
	If giPostFix = 1
		SetGadgetState(#CheckBox_PostFix, #PB_Checkbox_Checked) 
	EndIf
	;---------------------------------------------------------
